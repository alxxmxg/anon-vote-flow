...
const problematicas: Problematica[] = [
  { id: "mantenimiento", label: "Mantenimiento", description: "Falta de mantenimiento en aulas y áreas comunes.", icon: <Wrench className="w-5 h-5" /> },
  { id: "seguridad", label: "Seguridad", description: "Mejorar la seguridad dentro y fuera del campus.", icon: <Shield className="w-5 h-5" /> },
  { id: "equipo_obsoleto", label: "Equipo Obsoleto", description: "Actualización de equipo en laboratorios y biblioteca.", icon: <Monitor className="w-5 h-5" /> },
  { id: "oferta_academica", label: "Oferta Académica", description: "Ampliación de materias optativas y especializaciones.", icon: <BookOpen className="w-5 h-5" /> },
  { id: "transporte", label: "Transporte", description: "Mejorar rutas y horarios del transporte universitario.", icon: <Bus className="w-5 h-5" /> },
];
...
export default function BoletaVotacion() {
  ...
  const confirmVote = async () => {
    ...
    setError("");
    try {
      const { data, error: fnError } = await supabase.functions.invoke("cast-vote", {
        body: { problematicas: selected },
      });

      if (fnError) {
        setError("Error al procesar tu voto. Intenta de nuevo.");
        setLoading(false);
        return;
      }
            ...
            <button
              ...
              className={`w-full text-left p-4 rounded-xl border-2 transition-all duration-200 ${
                isSelected ? "bg-primary/5 border-primary shadow-sm"
                  : isDisabled ? "bg-muted/50 border-border opacity-50 cursor-not-allowed"
          ...
          <div className="w-full max-w-md bg-card rounded-2xl p-6 shadow-xl">
            ...
            <div className="bg-muted rounded-lg p-3 mb-6">
              {selected.map((id) => {
                ...
                return <p key={id} className="text-sm text-foreground py-1">• {p.label}</p>;
              })}
            </div>
            ...
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => setShowConfirm(false)} className="flex-1 h-11 rounded-xl">
                Cancelar
              </Button>
              <Button
                onClick={confirmVote}
